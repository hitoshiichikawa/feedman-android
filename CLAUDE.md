...
- 最低 SDK: API 26 (Android 8.0) / target は最新安定版
- アーキテクチャ: MVVM + Repository、単方向データフロー（`design/SPEC.md` §2 が正本）
- 非同期 / ページング: Coroutines + Flow / Paging 3
- ネットワーク: Retrofit + OkHttp + kotlinx.serialization
- 画像: Coil（favicon は data URL。`design/SPEC.md` §4.4）
- DI: Hilt
- 外部リンク: Chrome Custom Tabs
- ビルド: Gradle (Kotlin DSL) + Version Catalog（`gradle/libs.versions.toml`）
- テスト: JUnit4 + kotlinx-coroutines-test + Turbine + MockWebServer / Compose UI Test
- Lint / Format: Android Lint + ktlint
- CI: GitHub Actions（`./gradlew build` + unit tests）

---

## プロジェクト固有ドキュメント（実装前に必ず参照）

| ドキュメント | 役割 |
|---|---|
| `design/SPEC.md` | アプリ仕様の正本。API 契約（§4）・画面採用案（§5）・受け入れ基準（§10） |
| `design/SERVER.md` | サーバー側（`hitoshiichikawa/feedman`）のトークン認証・プッシュ通知仕様 |
| `docs/GRAND-DESIGN.md` | アーキテクチャ全体像。パッケージ構成・レイヤー責務・状態同期設計 |
| `design/IDD-CLAUDE-ISSUES.md` | Issue バックログと Epic/子Issue の対応表・粒度基準 |
| `design/Feedman Mobile.html` + `design/mobile/*.jsx` | 視覚・挙動のリファレンス（データはモック。API 形は SPEC §4 が正） |

- プロトタイプの Tweaks（複数案出し分け）は探索用。**実装は SPEC §5 の採用案のみ**。
- v1 スコープ外（キーワードプッシュ通知 / OPML / オフライン全文キャッシュ / フィード内検索 UI）を勝手に実装しない。
...
### Kotlin / Android プロジェクトの規約

- 関数は単一責務・**40 行以内**を目安とする（Composable は画面 / セクション / 部品で分割）
- 公開 API（repository インターフェース・共通 UI コンポーネント）には **KDoc** を付与する
- エラーは `FeedmanException`（API エラー `code` を保持）等の独自型で表現し、UI 層で `message` を表示する。silent fail を作らない
- 非同期は `suspend` 関数 + `Flow` を優先し、callback ネストを避ける。`GlobalScope` 禁止
- 状態は `ViewModel` の `StateFlow<UiState>` に集約し、Composable は state を受け取って描画するだけにする（stateless Composable 優先）
- パッケージ構成は `docs/GRAND-DESIGN.md` §3 に従う（`core/*` と `feature/*` の責務分離）
- リソース命名: 文字列は `strings.xml`（日本語が既定）。ハードコード文字列を Composable に直書きしない
- DI は Hilt module（`core/*/di`）に集約し、手動シングルトンを作らない
...
- **配置**: 単体テストは `app/src/test/`（JVM）、Compose UI / instrumented テストは `app/src/androidTest/`。ロジック検証は JVM 単体テストを最優先する
- **命名**: `` fun `<条件>のとき<期待結果>`() `` のバッククォート日本語名 or `methodName_condition_expected` 形式。クラスは `<対象>Test`
- **fixture 配置**: API レスポンス JSON は `app/src/test/resources/fixtures/` に集約し、テスト間で共有する
- **コルーチン**: `runTest` + `kotlinx-coroutines-test` を使い、`Thread.sleep` で待たない。`Flow` の検証は Turbine を使う
- **HTTP**: repository / APIClient のテストは MockWebServer で実レスポンス JSON を返す（Retrofit インターフェースをモックしない）
- **時刻**: 相対日時など時刻依存ロジックは `Clock` を注入して固定する
- **CI 前提**: 通常の Issue 実装で要求されるのは JVM 単体テスト。エミュレータが必要な instrumented テストを CI 必須にしない
...
- 本リポジトリでは以下の情報をコミットしない
  - Google OAuth クライアントシークレット・本番 API の認証情報
  - リリース署名鍵（keystore）と `keystore.properties` 等の実値
  - `google-services.json` などプッシュ通知用の実クレデンシャル（次フェーズで扱う際は CI secrets / ローカル配置とする）
  - 個人のアカウント情報・実ユーザーデータ
- API base URL の切替は `BuildConfig` + Gradle プロパティで行い、実トークンをコードに埋め込まない
