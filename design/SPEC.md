...
### 5.6 購読設定（ボトムシート）
- フェッチ間隔セグメント（30/60/180/360分）→ `PUT /api/subscriptions/{id}/settings`。
  - **15 分は廃止（2026-06-12 決定）**: サーバー側バリデーションが 30〜720 分・30 分刻み（`feedman#46`）のため。プロトタイプに残る 15 分表示は無視すること。
...
6. ✅ フェッチ間隔セグメント: **30/60/180/360 分**（2026-06-12 追記。15 分はサーバー側バリデーション 30〜720 分・30 分刻み＝`feedman#46` に適合しないため廃止）。
